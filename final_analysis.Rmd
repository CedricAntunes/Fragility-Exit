...
output:
  ...
  pdf_document:
    toc: yes
    toc_depth: 2
header-includes: \usepackage{lscape} \usepackage{pdfpages} \usepackage{graphicx} \usepackage[figuresright]{rotating}
---
...
final_clean_percentiles_data_normalized_1 <- final_clean_percentiles_data |>
  ...
  mutate(
    ...
    WGI_FRAGILE_IDEAL         = if_else(NEW_WGI_LOADING_FACTOR_1_NORMALIZED < NEW_WGI_30TH_PERCENTILE, 1, 0),
...
```{r, wgi-specification}
# Baseline empirical specification
wgi_form_baseline <- as.formula(
  paste("WGI_FRAGILE_IDEAL ~", paste(baseline_vars, 
                                     collapse = " + "))
)
...
# OLS --------------------------------------------------------------------------
# ------------------------------------------------------------------------------
linear_model <- feols(wgi_form_baseline, 
                      data = standardized_data_final)
...
# Logit ------------------------------------------------------------------------
# ------------------------------------------------------------------------------
logit_model  <- feglm(wgi_form_baseline, 
                      family = binomial(), 
                      data = standardized_data_final)
...
# Fixed-effects (within) + Driscoll–Kraay SEs ----------------------------------
# ------------------------------------------------------------------------------
fe_mod <- feols(wgi_form_baseline, 
                data = standardized_data_final,
                # Two-way FEs
                fixef = c("ISO_CODE_3",
                          "YEAR"))
...
# McFadden PR2 (pooled logit) - your original approach
logit_null <- glm(WGI_FRAGILE_IDEAL ~ 1,
                  family = binomial,
                  data   = standardized_data_final)
McFadden_R2 <- 1 - (as.numeric(logLik(logit_model)) /
                    as.numeric(logLik(logit_null)))
...
fixest::etable(
  ...
  tex = TRUE,
  fontsize = "footnotesize",
  notes = "All models use the same unbalanced WGI country-year panel (1996-2021). Columns (1) and (2) report pooled OLS and pooled logit estimates; column (3) reports a two-way FE linear model with country and year effects absorbed; column (4) reports a two-way FE logit with country and year fixed effects. Standard errors are two-way clustered by country and year. Driscoll-Kraay HAC SEs with a three-year bandwidth yield nearly identical inference and are shown in the appendix. All covariates are standardized; logit coefficients are log-odds. Significance: $^{***} p<0.01$, $^{**} p<0.05$, $^{*} p<0.1$.",
  title = "Linear, Logistic, and Fixed-Effects Estimates of WGI Fragility Status (1996–2021)"
)
```
